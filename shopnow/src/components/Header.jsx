  ...
  return (
    <header>
      <nav className='navbar navbar-expand-sm fixed-top bg-body-tertiary'>
        <Link className='navbar-brand ms-1' to='/'>Kirpal Mobile Care</Link>
        {userInfo ? (
        <div className='dropdown'>
          <nav className='nav-link dropdown-toggle' data-bs-toggle='dropdown'>{userInfo.name}</nav>
          <ul className='dropdown-menu'>
            <li className='dropdown-item'>User Profile</li>
            <li className='dropdown-item'>Order History</li>
            <li><hr className='dropdown-divider'/></li>
            <li className='dropdown-item'><Link to='/signin?redirect=/shipping' onClick={HandleSignout}>Sign out</Link></li>
          </ul>
        </div>
        ) : (<Link className='navbar-nav nav-link me-auto' to='/signin'>Sign in</Link>)}
